 ...
 *
 * <p>
 * Return a {@link BorrowResult} with two properties:
 * <ul>
 ...
 * <li>No valid card:
 * { allowed: false, message: "Invalid library card - please renew at the front
 * desk" }</li>
    ...
    public static BorrowResult canBorrowBook(int memberAge, boolean hasValidCard, int overdueBooks) {
        if (memberAge < 6) {
            return new BorrowResult(
                    false,
                    "Too young - must be at least 6 years old");
        } else if (!hasValidCard) {
            return new BorrowResult(
                    false,
                    "Invalid library card - please renew at the front desk");
