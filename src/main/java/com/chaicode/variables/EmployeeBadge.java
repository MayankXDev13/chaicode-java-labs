 ...
 *
 * <p>
 * HR needs a badge formatter for the new hire orientation. Given an employee
 * profile,
...
public final class EmployeeBadge {
    ...
    public static String formatBadge(EmployeeProfile profile) {
        ...
        String manager = profile.isManager() ? "MANAGER" : "STAFF";

        return "[ID-" + String.format("%05d", profile.employeeId()) + "] "
                + profile.name()
                + " | "
                + profile.department()
                + " | "
